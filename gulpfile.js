...
console.log('      CCZP Copyright  (C)  2016 Sam Wray      '+ "\n" +
            '----------------------------------------------'+ "\n" +
            '           Casio CZ Patcher  (CCZP)           '+ "\n" +
...
gulp.task('mkdir:media', ['clean'], function(cb) {
	/*return mkdirp('./dist/media/default').then(() => {

	}).catch(error => {
		cb(error);
	});*/

	return gulp.src('./media/default/**/*', {base: './media'})
			.pipe(gulp.dest('dist/media'));
});

gulp.task('nwjs', ['clean', 'ejs:nwjs', 'webpack', 'mkdir:media', 'copy', 'copy:nwjs:include', 'copy:nwjs:package', 'copy:nwjs:mediamanager', 'copy:nwjs:mediamanagermodules', 'clean:nwjs'], function(cb) {
...
gulp.task('watch', ['build', 'connect', 'set-watcher']);
